...
import { Settings, Palette, Database, Rocket, Box, Moon, Sun, ChevronLeft, LayoutDashboard, Puzzle, User } from 'lucide-react';
    ...
    `}>
      ...
      <div className={`p-4 flex items-center gap-4 border-b border-gray-100 dark:border-gray-800 h-[73px] ${isCollapsed ? 'justify-center' : ''}`}>
        ...
        <div className={`transition-opacity duration-200 overflow-hidden ${isCollapsed ? 'hidden' : 'block'}`}>
          <h1 className="font-bold text-gray-800 dark:text-white text-lg whitespace-nowrap">{appTitle}</h1>
          <p className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">{appSlogan}</p>
        </div>
      </div>
        ...
        <button
            ...
            className={`w-full flex items-center gap-3 rounded-xl transition-all duration-200 group relative
                ${activeTab === 'profile' 
                   ...
                   : 'hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
                }
                ${isCollapsed ? 'justify-center p-3' : 'px-3 py-3'}
            `}
        >
             <div className={`flex-shrink-0 ${activeTab === 'profile' ? '' : 'text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300'}`}>
                <User size={20} />
             </div>
             <span className={`whitespace-nowrap font-medium transition-all duration-200 ${isCollapsed ? 'hidden' : 'block'}`}>
                حساب کاربری
             </span>
             {isCollapsed && (
              <div className="absolute right-full mr-3 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 shadow-lg">
